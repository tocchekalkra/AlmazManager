...
export default function DocumentsPage() {
    ...
    async function cancelDocument(document: UnifiedDocument) {
        if (!isAdministrator || document.data.status !== 'Posted') {
            return;
        }
                        ...
                        <tbody>
                            {!loading && filteredDocuments.map((document) => (
                                ...
                                >
                                    ...
                                    <td>{new Date(document.data.createdAtUtc).toLocaleString('ru-RU')}</td>
                                    <td>
                                        {document.source === 'warehouse'
                                            ? [document.data.supplier, document.data.externalNumber]
                                                .filter(Boolean)
                                                .join(' · ') || '—'
                                            : '—'}
                                    </td>
            ...
            {selected && (
                <div style={styles.overlay} onMouseDown={() => setSelected(null)}>
                    <div style={styles.modal} onMouseDown={(event) => event.stopPropagation()}>
                                    ...
                                    {selected.source === 'warehouse'
                                        ? selected.data.items.map((item) => {
                                            ...
                                            return (
                                                <tr key={item.id}>
                                                    <td>
                                                        <strong>{material?.name ?? 'Архивный материал'}</strong>
                        ...
                        {isAdministrator && selected.data.status === 'Posted' && (
