...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest warehousemanager.web.vs -- --template=react-ts  --no-rolldown --no-immediate`.
- Обновление `vite.config.ts` с портом.
- Создание файла проекта (`warehousemanager.web.vs.esproj`).
