...
using WarehouseManager.Contracts.Requests.Materials;
...
{
    private readonly CreateMaterialHandler _createMaterialHandler;
    private readonly GetMaterialsHandler _getMaterialsHandler;
    private readonly GetMaterialCatalogHandler _getCatalogHandler;
    private readonly GetMaterialByIdHandler _getByIdHandler;
    private readonly UpdateMaterialHandler _updateMaterialHandler;
    private readonly SetMaterialActivityHandler _activityHandler;
        ...
        SetMaterialActivityHandler activityHandler)
    {
        ...
        _getMaterialsHandler = getMaterialsHandler;
        ...
        [FromBody] CreateMaterialRequest request)
    {
        ...
        var response =
            await _createMaterialHandler.HandleAsync(command);
    ...
    {
        var response =
            await _getMaterialsHandler.HandleAsync();

        return Ok(response);
    }

    [HttpGet("catalog")]
    public async Task<ActionResult<MaterialCatalogResponse>> GetCatalog(
        [FromQuery] MaterialCatalogRequest request)
    {
        var response =
        ...
        [FromBody] UpdateMaterialRequest request)
    {
        ...
        var response =
            await _updateMaterialHandler.HandleAsync(command);

        return Ok(response);
    }

    [HttpPatch("{id:guid}/archive")]
    public async Task<ActionResult<MaterialResponse>> Archive(Guid id)
    {
        var command = new SetMaterialActivityCommand(
            id,
            false);

        var response =
            await _activityHandler.HandleAsync(command);

        return Ok(response);
    }

    [HttpPatch("{id:guid}/restore")]
    public async Task<ActionResult<MaterialResponse>> Restore(Guid id)
    {
        var command = new SetMaterialActivityCommand(
            id,
            true);
